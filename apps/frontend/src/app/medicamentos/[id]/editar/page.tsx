...
export default function EditarMedicamentoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.error) {
        throw new Error((response as any).message || response.error || 'Falha ao atualizar medicamento')
      }

      router.push('/medicamentos')
    } catch (err: any) {
      setError(err.message || 'Ocorreu um erro ao atualizar o medicamento.')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (initialLoading) {
    return (
      <div className="bg-slate-50 font-display flex items-center justify-center min-h-screen">
        <span style={{ color: 'var(--color-text-secondary)' }}>Carregando dados...</span>
      </div>
    )
  }
          ...
          <main className="flex flex-1 justify-center py-10 px-4">
            <div className="flex flex-col max-w-[640px] flex-1">
              ...
              <div className="flex flex-col gap-2 mb-10 px-2 text-center">
                <h1 className="text-4xl font-black tracking-tight" style={{ fontSize: '2rem', fontWeight: 900, color: 'var(--color-text-primary)' }}>Editar Medicamento</h1>
                <p className="text-lg" style={{ color: 'var(--color-text-secondary)' }}>Atualize os detalhes do seu tratamento.</p>
              </div>
